...
class TestChangelogRequest:
    def test_when_created_with_from_version_then_stores_values(self) -> None:
        request = ChangelogRequest(from_version="1.0.0")

        assert request.from_version == "1.0.0"
        assert request.dry_run is False

    def test_when_dry_run_true_then_stored(self) -> None:
    ...
    def test_equality_when_different_version_then_not_equal(self) -> None:
        r1 = ChangelogRequest(from_version="1.0.0")
...
class TestChangelogResponse:
    def test_when_created_with_entries_then_stores_all(self) -> None:
        entries = ["- feat: add login", "- fix: bug"]
        response = ChangelogResponse(entries=entries)

        assert response.entries == entries

    def test_when_created_with_empty_entries_then_stores_empty(self) -> None:
    ...
    def test_equality_when_same_entries_then_equal(self) -> None:
        r1 = ChangelogResponse(entries=["- feat: x"])
        r2 = ChangelogResponse(entries=["- feat: x"])

        assert r1 == r2

    def test_equality_when_different_entries_then_not_equal(self) -> None:
        r1 = ChangelogResponse(entries=["- feat: x"])
