...
class TestReleaseBranchExistsError:
    ...
    def test_when_created_then_stores_branch_name(self, branch_name: str) -> None:
        error = ReleaseBranchExistsError(branch_name)

        assert error.branch_name == branch_name

    def test_when_created_then_is_exception(self) -> None:
        error = ReleaseBranchExistsError("release/v1.0.0")

        assert isinstance(error, Exception)

    def test_when_created_then_is_catchable(self) -> None:
